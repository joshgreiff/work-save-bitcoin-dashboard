...
describe("closing snapshot", () => {
  it("keeps Episode 1 opening value while current portfolio uses the close", () => {
    const episodes = loadEpisodes().episodes;
    const portfolio = loadPortfolio();
    expect(episodes[0]?.portfolioValueCents).toBe(199991);
    expect(portfolio.currentPortfolioValueCents).toBe(199692);
    expect(portfolio.currentValuationAt).toBe("2026-09-16T16:00:00-04:00");
    expect(
